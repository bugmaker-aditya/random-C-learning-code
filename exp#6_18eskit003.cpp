...
char CalFirst[10][100], calc_follow[10][100];
char production[10][10],f[10], first[10],ck;
int main(){
    int jm = 0, km = 0,i=0, choice, kay, ptr=-1;
    int point1 = 0, point2, xxx, land = 0;
    count = 8;
    char c, ch, done[count];
    FILE *fptr ;
    fptr = fopen("xyz.txt", "r");
    while(fgets(production[i], 128, fptr))   {
        production[i][strlen(production[i]) - 1] = '\0';
        i++;}
    for(k = 0; k < count; k++)
        for(kay = 0; kay < 100; kay++)          CalFirst[k][kay] = '!';
    for(k = 0; k < count; k++) {
        c = production[k][0];
        point2 = 0, xxx = 0;
...
void followfirst(char c, int c1, int c2){    int k;
    if(!(isupper(c)))        f[m++] = c;
    else{     int i = 0, j = 1;
          for(i = 0; i < count; i++)
            if(CalFirst[i][0] == c)           break;
          while(CalFirst[i][j] != '!')
            if(CalFirst[i][j] != ':')   f[m++] = CalFirst[i][j];
