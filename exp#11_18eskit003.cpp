...
int leadset(int cnt, int idx){
		char nterm;
		int k=0;
		for(int i=0; i<cnt ;i++){	nterm=prodn[i][0];
			while (idx != '\n'){
				if(prodn[i][idx] != nterm && (prodn[i][idx] >='A' || prodn[i][idx] <= 'Z'))
					...
					if( (prodn[i][idx]=='(')  || (prodn[i][idx]==')') || (prodn[i][idx]=='*') || (prodn[i][idx]=='+') ||  (prodn[i][idx]=='-') || (prodn[i][idx]=='/') ){
						lead[i][k]=prodn[1][idx];						k=k+1;						}
				else if( (prodn[i][idx]=='i') && (prodn[i][idx+1]=='d')){
								lead[i][k]=prodn[i][idx];
								lead[i][k+1]=prodn[i][idx+1];				k=k+1	;				}
			  else				return 0;
			}		idx = 3;
	}	}
int main (){	FILE *fp;
